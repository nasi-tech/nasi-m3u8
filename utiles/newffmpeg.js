...
exports.transcode = function(movie) {
    ...
    ffmpeg.ffprobe(path, function(err, metadata) {
        ...
        Setting.find().exec(function(err, setting) {
            ...
            if (videooriginH <= 720 * 1 && videooriginC == 'h264' && audiooriginC == 'aac') {
                chunk(path, des, id, config, vf, false);
            } else {
...
function ffmpegtransandchunk(des, path, config, vf, id) {
    ffmpeg(path).addOptions(config).output(des + '/index.m3u8').on('start', function() {
        Movie.findOne({
            _id: id
...
function chunk(path, des, id, config, vf, tsjiami) {
    ...
    ffmpeg(path).addOptions(chunkconfig).output(des + '/index.m3u8').on('end', function() {
        screenshots(path, des);
        Movie.findOne({
            _id: id
        }).exec(function(err, movie) {
...
function togif(path, des, setting) {
    ...
    ffmpeg(path).addOptions([
        ...
        '-r 15',
        '-vf scale=' + gifwidth + ':-1'
    ]).output(des + '/1.gif').on('end', function() {
        fs.unlinkSync(path);
    }).run();
    // } else {
    //     fs.unlinkSync(path);
